...
part 'senshu_gakuren_data.g.dart'; // このファイル名に合わせてください

@HiveType(typeId: 10) // ユニークなtypeIdを指定
class Senshu_Gakuren_Data extends HiveObject {
  @HiveField(0)
  ...
  factory Senshu_Gakuren_Data.initial({
    required int id,
    int univid = 0,
    int gakunen = 0,
  }) {
  ...
  // --- 🌟 通常の選手データから学連選抜用の選手データへ変換するための特別なファクトリコンストラクタを追記 🌟 ---
  // List型のフィールドをディープコピー（値渡し）で初期化するように修正
  factory Senshu_Gakuren_Data.fromSenshuData(SenshuData senshu) {
    return Senshu_Gakuren_Data(
      id: senshu.id,
  ...
  // Senshu_Gakuren_Data クラス内 (修正後の toJson() のみ)
  ...
  factory Senshu_Gakuren_Data.fromJson(Map<String, dynamic> json) {
    return Senshu_Gakuren_Data(
      id: json['id'] as int,
