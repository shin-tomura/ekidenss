...
import 'package:ekiden/senshu_data.dart'; // 追記
part 'senshu_r_data.g.dart'; // このファイル名に合わせてください

@HiveType(typeId: 6) // ユニークなtypeIdを指定 (SenshuDataは0、UnivDataは1、Ghensuuは2)
class Senshu_R_Data extends HiveObject {
  @HiveField(0)
  ...
  int sijiflag; //卒業年
  @HiveField(48)
  int sijiseikouflag; //100で自分で追加した選手、自分で削除しない限り削除されないようにするため
  @HiveField(49)
  ...
  factory Senshu_R_Data.initial({
    required int id,
    int univid = 0,
    int gakunen = 0,
  }) {
    return Senshu_R_Data(
      id: id,
  ...
  // --- 🌟 現役データから卒業データへ変換するための特別なファクトリコンストラクタを追記 🌟 ---
  // List型のフィールドをディープコピー（値渡し）で初期化するように修正
  factory Senshu_R_Data.fromSenshuData(SenshuData senshu) {
    return Senshu_R_Data(
      id: senshu.id,
      univid: senshu.univid,
      gakunen: senshu.gakunen,
      name: senshu.name,
      name_tanshuku: senshu.name_tanshuku,
      magicnumber: senshu.magicnumber,
      a: senshu.a,
      b: senshu.b,
      sositu: senshu.sositu,
      sositu_bonus: senshu.sositu_bonus,
      seichoutype: senshu.seichoutype,
      genkaitoppakaisuu: senshu.genkaitoppakaisuu,
      seichoukaisuu: senshu.seichoukaisuu,
      genkaichokumenkaisuu: senshu.genkaichokumenkaisuu,
      mokuhyo_b: senshu.mokuhyo_b,
      rirontime5000: senshu.rirontime5000,
      rirontime10000: senshu.rirontime10000,
      rirontimehalf: senshu.rirontimehalf,
      kiroku_nyuugakuji_5000: senshu.kiroku_nyuugakuji_5000,
      // **値渡し（ディープコピー）に修正**
      time_bestkiroku: List.of(senshu.time_bestkiroku),
      year_bestkiroku: List.of(senshu.year_bestkiroku),
      month_bestkiroku: List.of(senshu.month_bestkiroku),
      zentaijuni_bestkiroku: List.of(senshu.zentaijuni_bestkiroku),
      gakunaijuni_bestkiroku: List.of(senshu.gakunaijuni_bestkiroku),

      konjou: senshu.konjou,
      heijousin: senshu.heijousin,
      choukyorinebari: senshu.choukyorinebari,
      spurtryoku: senshu.spurtryoku,
      kegaflag: senshu.kegaflag,
      hirou: senshu.hirou,
      kaifukuryoku: senshu.kaifukuryoku,
      anteikan: senshu.anteikan,
      chousi: senshu.chousi,
      karisuma: senshu.karisuma,
      kazetaisei: senshu.kazetaisei,
      atusataisei: senshu.atusataisei,
      samusataisei: senshu.samusataisei,
      noboritekisei: senshu.noboritekisei,
      kudaritekisei: senshu.kudaritekisei,
      noborikudarikirikaenouryoku: senshu.noborikudarikirikaenouryoku,
      tandokusou: senshu.tandokusou,
      paceagesagetaiouryoku: senshu.paceagesagetaiouryoku,

      // List<List<T>>のようなネストされたリストの場合、外側のリストのみがコピーされ、内側のリストは参照のまま残るため、ディープコピーを行う
      entrykukan_race: senshu.entrykukan_race
          .map((innerList) => List<int>.of(innerList))
          .toList(),
      kukanjuni_race: senshu.kukanjuni_race
          .map((innerList) => List<int>.of(innerList))
          .toList(),
      kukantime_race: senshu.kukantime_race
          .map((innerList) => List<double>.of(innerList))
          .toList(),
      ...
      // **値渡し（ディープコピー）に修正**
      kukannaijuni: List.of(senshu.kukannaijuni),

      temp_juni: senshu.temp_juni,
      chokuzentaikai_pbflag: senshu.chokuzentaikai_pbflag,
      chokuzentaikai_kojinrekidaisinflag:
          senshu.chokuzentaikai_kojinrekidaisinflag,
      chokuzentaikai_kojinunivsinflag: senshu.chokuzentaikai_kojinunivsinflag,
      chokuzentaikai_zentaikukansinflag:
          senshu.chokuzentaikai_zentaikukansinflag,
      chokuzentaikai_univkukansinflag: senshu.chokuzentaikai_univkukansinflag,
      string_racesetumei: senshu.string_racesetumei,
    );
  }
  ...
  // Senshu_R_Data クラス内 (修正後の toJson() のみ)
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      // ★修正: 単体の double に _roundDouble を適用
      'magicnumber': _roundDouble(magicnumber),
      'a': _roundDouble(a),
      'b': _roundDouble(b),
      'sositu': sositu,
      ...
      'mokuhyo_b': mokuhyo_b,
      // ★修正: 単体の double に _roundDouble を適用 (理論タイム、入学時記録)
      'rirontime5000': _roundDouble(rirontime5000),
      'rirontime10000': _roundDouble(rirontime10000),
      'rirontimehalf': _roundDouble(rirontimehalf),
      'kiroku_nyuugakuji_5000': _roundDouble(kiroku_nyuugakuji_5000),
      // ★修正: List<double> に _roundDoubleList を適用
      'time_bestkiroku': _roundDoubleList(time_bestkiroku),
      ...
      'kukantime_race': _roundDoubleNestedList(kukantime_race),
      // ★修正: 単体の double に _roundDouble を適用
      'time_taikai_total': _roundDouble(time_taikai_total),
      'speed': _roundDouble(speed),
      'sijiflag': sijiflag,
  ...
  factory Senshu_R_Data.fromJson(Map<String, dynamic> json) {
    return Senshu_R_Data(
      id: json['id'] as int,
